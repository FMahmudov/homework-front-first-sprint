...
import { FilterValues, UserList2 } from './UserList2';
...
export const HW2 = () => {
  ...
  const filterUsers = (filter: FilterValues) => {

    setFilter(filter)
   
  };

  const handleCurrentUsers = () => {
    if(filter === 'All'){
      setCurrentUsers({
        myFriends: users.myFriends
      })
    }
    else {
      const filteredUsers = currentUsers.myFriends.filter(friend => friend.address.city === filter)
      setCurrentUsers({
